...
export function handler(fn) {
  return async (req, res) => {
    try {
      ...
      // Kesalahan pengaturan (Environment Variables) ditampilkan apa adanya
      // supaya mudah diperbaiki. Kesalahan lain tetap disamarkan.
      const msg = String(err?.message || "");
      if (msg.includes("belum diisi")) {
        return res.status(500).json({ error: `Pengaturan server belum lengkap: ${msg}. Perbaiki di Vercel lalu Redeploy.` });
      }
